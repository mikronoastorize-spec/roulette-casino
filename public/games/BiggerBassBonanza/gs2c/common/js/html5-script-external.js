  ...
  var b = { LOWER: 0, UPPER: 1 },
    ...
    j = function (a, f, b) {
      ...
      for (var c = {}, a = a.split(f), d = 0; d < a.length; ++d)
        (f = a[d].split(b, 2)),
          (c[f[0]] =
            1 < f.length ? decodeURIComponent(f[1]).replace(/\+/g, " ") : "");
      return c;
    };
  return {
    CASE: b,
    ...
    getObjectValues: function (a) {
      var b = [];
      if (c(a)) for (var d in a) e(a[d]) || b.push(a[d]);
      return b;
    },
    ...
    changeCase: function (a, c, e) {
      if (!g(a) || 0 == $.trim(a).length) return a;
      var h = !1;
      d(c) && (h = !0);
...
var Html5GameManager = (function () {
  var b = {},
        ...
        if (Util.isObject(d)) {
          ...
          switch (d.common) {
            case "EVT_GET_CONFIGURATION":
              ...
              try {
                Util.isFunction(window.sendToGame)
                  ? window.sendToGame(e)
                  : console.log(
                      "sendMessageToGame",
                      "sendToGame is not function"
                    );
              } catch (k) {
    ...
    g = function (b, d) {
      UHT_DEVICE_TYPE.MOBILE && d
        ? /^js:\/\/.*/i.test(d)
          ? new Function(d.substring(5, d.length))()
          : d && window.open(d, "_self")
        : b &&
          (/^js:\/\/.*/i.test(b)
            ? new Function(b.substring(5, b.length))()
            : b && window.open(b, "_self"));
    };
  extendSessionRequest = function () {
    if (b.extendSessionUri)
      for (
        var c = b.extendSessionUri.split(","),
          d = ["_t", new Date().getTime()].join("="),
          e = 0;
        e < c.length;
        e++
      )
      ...
      if (d) {
        if (!c.hasOwnProperty("historyType") || "external" != c.historyType)
          (d += "&tz=" + encodeURIComponent(TimeUtil.getTimeZoneOffset())),
            (c.HISTORY = d);
        Util.extend(b, { gameConfig: c });
      }
      b.extendSessionInterval &&
        b.extendSessionUri &&
        (i = setInterval(extendSessionRequest, b.extendSessionInterval));
